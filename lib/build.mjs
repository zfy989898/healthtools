...
// JSON-LD dateModified must state the same day the sitemap does, so it is filled in after the
// page hash is resolved; hashing the token keeps the hash independent of the date itself.
const LASTMOD_TOKEN = '@@LASTMOD@@'
// Google reads publisher/entity markup as the site's own claim about who is speaking.
// Every field here is verifiable from the site itself: the logo is a file that exists,
// the sameAs target is the repository the source is published in. Nothing is invented -
// no founding date, no postal address, no telephone number we do not have.
...
const ORG_NODE = {
  ...
  sameAs: 'https://github.com/zfy989898/healthtools',
}
...
const FIELD_LABELS = {
  ...
  whtr: 'Waist-to-height ratio', status: 'Against the published 0.5 boundary value',
  boundaryWaist: 'Waist measurement that sits exactly at 0.5',
  waistVsBoundary: 'Your waist minus that measurement (+ is above it)',
  ree: 'Resting energy expenditure', equationUsed: 'Equation used',
  weightTerm: 'Weight term', heightTerm: 'Height term', ageTerm: 'Age term',
  sexTerm: 'Sex constant', total: 'Sum of the four terms',
  reeFromRegression: 'Same equation, unsimplified regression coefficients',
  simplificationGap: 'Difference between the two published forms',
  studyRange: 'Age against the source study range',
  hrr1: 'Heart rate recovery at one minute',
  vsCutOff: 'Against the published cut-off',
  cutOffNote: 'How the boundary is defined',
  hrRise: 'Rise from resting to peak',
  cohortNote: 'Where the cut-off comes from',
  fallInSecondMinute: 'Fall during the second minute',
  totalTwoMinutes: 'Fall from peak to two minutes',
  secondMinuteNote: 'What the second-minute figure means',
  twoMinuteNote: 'Second-minute readings',
}
...
<meta property="og:image:alt" content="HealthTools - free calculators that print the formula they used and the publication it came from.">
...
<p><a href="/">All tools</a> &middot; <a href="/about/">About</a> &middot; <a href="/methodology/">Methodology</a> &middot; <a href="/privacy/">Privacy</a> &middot; <a href="/disclaimer/">Disclaimer</a> &middot; <a href="https://github.com/zfy989898/healthtools">Source code and formula citations</a></p>
</footer>
</body>
</html>
`
  ...
  const jsonld = [
    {
      ...
      inLanguage: 'en-US', isAccessibleForFree: true, dateModified: LASTMOD_TOKEN,
...
async function main() {
  ...
  for (const [slug, [name, desc, body]] of Object.entries(INFO)) {
    ...
    const html = shell({
      ...
      // Only /about/ is an AboutPage; labelling the privacy and contact pages that way is
      // a claim about the page that the page does not support.
      jsonld: [ORG_NODE, { '@context': 'https://schema.org', '@type': slug === 'about' ? 'AboutPage' : 'WebPage', name, url: `${ORIGIN}/${slug}/`, description: desc, publisher: ORG_REF }],
  ...
  const home = shell({
    ...
    jsonld: [ORG_NODE, { '@context': 'https://schema.org', '@type': 'WebSite', name: SITE, url: `${ORIGIN}/`, inLanguage: 'en-US', publisher: ORG_REF }],
...
`)
  ...
  for (const p of pages) {
    if (!p.html.includes(LASTMOD_TOKEN)) continue
    writeFileSync(join(dist, p.slug, 'index.html'), p.html.replaceAll(LASTMOD_TOKEN, store[p.slug]?.date ?? TODAY))
  }
  ...
  // This copy ships the ownership file blank - see README.
  writeFileSync(join(dist, 'BingSiteAuth.xml'), '<?xml version="1.0"?>\n<users>\n\t<user></user>\n</users>\n')
  ...
  // Google's own favicon guidance asks for a 48x48 .ico it can fetch; without one a
  // search result falls back to a generic globe, and every browser that ignores
  // rel="icon" logs a 404. og:image needs PNG - social crawlers do not fetch SVG.
  for (const f of ['favicon.ico', 'apple-touch-icon.png', 'og-image.png']) {
    copyFileSync(join(ROOT, 'src/assets', f), join(dist, f))
  }

  // Pages gives HTML `max-age=0, must-revalidate`, so every visit - and every
  // Googlebot fetch - is an origin round trip (cf-cache-status: DYNAMIC, ~0.6s TTFB
  // measured 2026-09-23). These are static, identical for all visitors, and Pages
  // purges the edge cache on each deploy, so the documents can be cacheable.
  // Paths must be listed per route: _headers has no wildcard here that would cover
  // both the pages and the long-lived assets, and the assets need the opposite TTL.
  const HTML_TTL = 'public, max-age=300, stale-while-revalidate=3600'
  const ASSET_TTL = 'public, max-age=604800'
  const headerRules = [
    ['/', HTML_TTL],
    ...pages.filter(p => p.slug !== 'index').map(p => [`/${p.slug}/`, HTML_TTL]),
    ['/styles.css', ASSET_TTL],
    ...['favicon.svg', 'favicon.ico', 'apple-touch-icon.png', 'og-image.png'].map(f => [`/${f}`, ASSET_TTL]),
    ['/robots.txt', 'public, max-age=86400'],
    ['/sitemap.xml', 'public, max-age=1800'],
  ]
  writeFileSync(join(dist, '_headers'), headerRules.map(([path, ttl]) => `${path}\n  Cache-Control: ${ttl}\n`).join('\n'))
